...
# # Example 2: Calculating Spectral Irradiance using SPCTRAL 2
# 
...
spc2rural = LightSource(source_type='SPECTRAL2', precipwater=1.0,turbidity=0.2, aod_model='rural', x=wl * 1e9)
spc2marit = LightSource(source_type='SPECTRAL2', precipwater=1.0,turbidity=0.2, aod_model='maritime', x=wl * 1e9)
spc2tropo = LightSource(source_type='SPECTRAL2', precipwater=1.0,turbidity=0.2, aod_model='tropospheric', x=wl * 1e9)
...
# 
# One of the most common uses for a spectral irradiance model such as SPCTRAL2 is to calculate how a particular solar cell technology behaves under varying spectral conditions during the day.  This is particularly important whan working with series connected tandem solar cells where the current matching condition will vary according to the incident spectrum.

# Here we plot the spectral irradiance at 12pm, 2pm, 3pm, 4pm, 5pm, 6pm and 7pm.  Note the strong relative loss in short-wavelength light relative to the infrared as the air-mass increases throughout the afternoon.  
...
for h in hours:
    spc2 = LightSource(source_type='SPECTRAL2', dateAndTime=datetime.datetime(2011, 6, 30, h, 00),precipwater=1.0,turbidity=0.05, x=wl * 1e9)
