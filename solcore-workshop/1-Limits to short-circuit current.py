...
# # Example 1: Integration for limiting current, limiting voltage model, efficiency limit
...
# 
# Trivich & Flinn assumed that the ideal solar cell would absorb all photons with energy above the band-gap energy Eg and that the _maximum_ voltage that could ever be attained in a solar cell is the band-gap energy.  The latter assumption is incorrect unless the solar cell is operating at absolute zero temperature.
# 
...
# 
# Now that the limit to Jsc is known, we can estimate the power of delivered by the solar cell by evaluating $\frac{Eg J_{sc}}{b}$
...
plt.figure(3)
plt.title('Limiting Efficiency power curve for Eg=1.42eV (GaAs)')
plt.plot(v,v*j)
plt.xlim(0.2, 1.2)
plt.ylim(0,350)
plt.xlabel('Voltage (V)')
...
plt.figure(1)
plt.title('Limiting Efficiency IV curve for Eg=1.42eV')
...
plt.figure(3)  # Plot the results calculated above:
...
ax.plot(eg,p1x/am15d1x.power_density,label='AM1.5d 1x', color=colors[0])
ax.axvline(eg[np.argmax(p1x)], color=colors[0])

ax.plot(eg,p30x/am15d30x.power_density,label='AM1.5d 30x', color=colors[1])
ax.axvline(eg[np.argmax(p30x)], color=colors[1])

ax.plot(eg,p1000x/am15d1000x.power_density,label='AM1.5d 1000x', color=colors[2]) 
